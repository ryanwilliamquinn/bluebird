...
module.exports = function (grunt) {
    ...
    function checkAscii(path, contents) {
        ...
        contents.split("\n").forEach(function (line, i) {
            // check if character is outside of the ascii character set based on 'notAscii' regex
    ...
    function build(paths, isCI) {
        var fs = require("fs");
        // readConstants takes the following arguments:
        // source code of the constants file as a string
        // path to the constants ('./src/constants.js')
        astPasses.readConstants(fs.readFileSync(CONSTANTS_FILE, "utf8"), CONSTANTS_FILE);

        // optional paths and mandatory paths are arrays of file paths, all in the ./src/ directory
        if (!paths) {
            paths = optionalPaths.concat(mandatoryPaths);
        }

        // getOptionalRequireCode takes the 'paths' array of file paths, with the ./src/ bit removed
        // then it checks if there are any variables the file may need injecting into its require statement
        //
        ...
        var sources = paths.map(function (v) {
            // nfcall is a promisifying function
            var promise = Q.nfcall(fs.readFile, v, "utf8")
                .then(function (contents) {
                    // checkascii does what it seems to do.  it throws if it finds a non-ascii character
            ...
            // QUESTION
            // this is a confusing piece of code.  seems like it doesn't need the first ret.sourceCode part?
            // how would it be different if it was just:
            // promise.then(function (v) { ret.sourceCode = v });

            // using v here is confusing!  v in one scope is the filepath,
            // v in the promise.then scope is the file contents
        ...
        // QUESTION
        // what is the point of promises.slice() here?

        // QUESTION
        // this is kind of effed up too.  promises doesn't return anything, it is just being as a marker for when
        // the operations are finished.  why not return ret in the promises,
        // what is the point of keeping another array (sources)?
    ...
    function testRun(testOption, jobs) {
        ...
        if (testOption === "aplus") {
            ...
            if (testOption !== "all" && !fs.existsSync("./test/" + files[0])) {
